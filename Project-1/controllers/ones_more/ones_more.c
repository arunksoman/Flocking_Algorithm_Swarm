...
#define MAX_SENS 3000      // Maximum sensibility value
#define MAX_SPEED 4.28       // Maximum speed
...
//int e_puck_matrix[16] =   {32,18,12,6, -2,-10,-12,-16,-24,-16,-10,12, 27,8,10,18}; // for obstacle avoidance giving more weigth (asymetry) for the right side
...
void process_received_ping_messages(void)
{
    ...
    while (wb_receiver_get_queue_length(receiver2) > 0) {
       ...
       // printf("theta is %f",theta);
        ...
        relative_speed[other_robot_id][0] = (DELTA_T)*(relative_pos[other_robot_id][0]-prev_relative_pos[other_robot_id][0]);
        relative_speed[other_robot_id][1] = (DELTA_T)*(relative_pos[other_robot_id][1]-prev_relative_pos[other_robot_id][1]);		
...
int main(){ 
	...
	msr -= msr*max_sens/(2*MAX_SENS);
      ...
      msr += bmsr;
       ...
       msl_w = msl*MAX_SPEED/124;	// Transforme the wheel speed for the real E-puck in webots ones
       msr_w = msr*MAX_SPEED/124;          
      // Set speed
